...
const AssetManager = (editor, args = {}) => {
    ...
    editor.Commands.add('open-save-modal', {
        run: function (editor, sender, options = {}) {
            ...
            const content = $(
                ...
                <div class='flex'><button class='ms-auto btn btn-primary'>Save</button></div>
            </div>`);
            ...
            for (let i in args.params) {
                ...
                const com = $(document.createElement(param.type));
                com.attr('name', param.name);
                com.attr('id', param.name);
                com.attr('value', param.value || '');
                com.attr('placeholder', param.placeholder|| 'Type Here...');
                com.addClass('text-input');
                if (param.required) {
                    com.attr('required', true);
                }

                const label = $(`<label for='${param.name}'>${param.label}</label>`);
                container.append(label, com);
                content.find('#container').append(container);
...
const initEditor = (args = {}) => {
    ...
    var editor = grapesjs.init({
        ...
        container: '#gjs',
        height: '100vh',
        plugins: [ModuleLoader, gjsBasic, gjsForms, gjsTailwind],
    ...
    AssetManager(editor, option.assets);

    Controls(editor, option);
